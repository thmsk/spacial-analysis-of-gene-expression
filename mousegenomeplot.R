...
mouse.genome.plot<-function(chromosomes, data, dataname)
{
#load data like this
#chromosomes<-read.table("/media/Data1000/data/seqs/M.musculus/mm9/mm9_R.genome")[,1:4]
...
y<-c(y,1,1,1,1);
scale<-1000000;
plot(x, y, type="n", xlab="", ylab="", axes=F, main=title, cex.main=2.5); #plotting an empty frame
#color coding
values=sites[,4];
...
cols<-colorRampPalette(c("steelblue4","white", "firebrick4"))(length(quants))
colors<-0;
# logspace
...
for(i in 1:length(chrom[,1])){
  ...
  text(1, coord1+0.75, labels=name, cex=1.1);
